/**
 * AI Service for CounselFlow
 * Real integration with backend AI agents
 */
...
class AIService {
  ...
  async getAvailableAgents(): Promise<AIAgent[]> {
    return this.makeRequest<AIAgent[]>('/api/v1/ai-agents/');
  }

  async submitQuery(request: AIQueryRequest): Promise<AIQueryResponse> {
    return this.makeRequest<AIQueryResponse>('/api/v1/ai-agents/query', {
      method: 'POST',
      body: JSON.stringify(request),
    });
  }

  async getQueryResult(taskId: string): Promise<AIQueryResponse> {
    return this.makeRequest<AIQueryResponse>(`/api/v1/ai-agents/query/${taskId}`);
  }

  async legalResearch(query: string, jurisdiction?: string, caseTypes?: string[]): Promise<AIQueryResponse> {
    return this.makeRequest<AIQueryResponse>('/api/v1/ai-agents/legal-research', {
      method: 'POST',
      body: JSON.stringify({
        query,
        jurisdiction,
  ...
  async analyzeContract(contractText: string, analysisType = 'comprehensive', riskAssessment = true): Promise<AIQueryResponse> {
    return this.makeRequest<AIQueryResponse>('/api/v1/ai-agents/contract-analysis', {
      method: 'POST',
      body: JSON.stringify({
  ...
  async pollQueryResult(taskId: string, maxAttempts = 30, intervalMs = 2000): Promise<AIQueryResponse> {
    let attempts = 0;
